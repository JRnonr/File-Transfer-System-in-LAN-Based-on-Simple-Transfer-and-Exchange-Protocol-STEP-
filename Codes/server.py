...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_DOWNLOAD:
        ...
        if block_index >= total_block:
            logger.error(f'<-- The "block_index" exceed the max index.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 405, TYPE_FILE, f'The "block_index" exceed the max index.', {}))
            return
        if block_index < 0:
            logger.error(f'<-- The "block_index" should >= 0.')
            connection_socket.send(
...
def tcp_listener(server_ip, server_port):
    ...
    # Determine the SOCK of the server transport protocol_ STREAM represents the streaming protocol TCP. If it is SOCK_ DGRAM stands for UDP protocol
    server_socket = socket(AF_INET, SOCK_STREAM)
    # To prevent the address port from being occupied and not released during startup, you can add it before bind
    server_socket.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
    # Bind the server IP and PORT, so that the client can access the server through the IP and port
    ...
    while True:
        try:
            # If a client connects, accept the client's connection, and return the client socket communication object and the client's IP address
